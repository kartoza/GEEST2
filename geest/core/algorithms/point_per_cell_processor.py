...
class PointPerCellProcessor:
    ...
    Example:
        ```python
        processor = PointPerCellProcessor(points_layer, '/path/to/working_dir', '/path/to/your/geopackage.gpkg')
    ...
    def __init__(
        ...
        self.gpkg_path = gpkg_path  # Store the GeoPackage path as a class property

        # Load the grid layer from the GeoPackage
        self.grid_layer = QgsVectorLayer(
            f"{gpkg_path}|layername=study_area_grid", "study_area_grid", "ogr"
        )
        if not self.grid_layer.isValid():
            raise QgsProcessingException(
                "Failed to load 'study_area_grid' layer from the GeoPackage."
            )
    ...
    def _select_features(
        self, layer: QgsVectorLayer, grid_layer: QgsVectorLayer, output_name: str
    ) -> QgsVectorLayer:
        """
    ...
    def _select_grid_cells(
        ...
        """
        Select grid cells that intersect with points using the native:extractbylocation algorithm.
        ...
        # Use processing algorithm to select grid cells that intersect with points
        params = {
            "INPUT": grid_layer,
            "PREDICATE": [0],  # Intersects predicate
            "INTERSECT": points_layer,  # Layer containing points
            "OUTPUT": output_path,
        }

        # Run the processing algorithm to extract grid cells by location (intersection with points)
        result = processing.run("native:extractbylocation", params)

        # Return the new QgsVectorLayer with the selected grid cells
        return QgsVectorLayer(result["OUTPUT"], "selected_grid_cells", "ogr")
    ...
    def _rasterize_grid(
        ...
        params = {
            ...
            "UNITS": 1,  # UNITS=1 indicates that the resolution is specified in map units (e.g., meters)
            "WIDTH": 100,  # Calculate number of pixels along x
            "HEIGHT": 100,
            "OUTPUT": output_path,
        }
